...
我深耕保险金融软件测试，专注软件质量保障，方向覆盖**接口测试、Web 自动化测试**与**测试开发（质量平台）**。近几年重点关注 **AI 赋能软件测试**，持续探索如何用 AI 提升测试设计效率、自动化覆盖率与缺陷分析质量。我相信：好的质量不是靠"点得多"，而是靠体系化的测试设计、可持续的自动化能力，以及对业务和技术的深入理解。

## 技术栈

- **接口测试**：Postman / Apifox / JMeter，接口自动化与数据校验
- **Web 自动化**：Playwright，UI 自动化与回归测试体系
- **测试开发**：Python / Pytest，自研测试工具与质量平台、CI 集成
- **数据库**：MySQL / Oracle / Dameng，测试数据构造与结果校验
- **环境**：Linux，测试环境搭建与运维脚本
